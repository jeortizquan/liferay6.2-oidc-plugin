...
public class LibFilter {
    ...
    protected String getUserGroupsInfoFromGraph(String userMembershipUrl, String accessToken) {
        try {
            URL url = new URL(userMembershipUrl);
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();

            connection.setRequestProperty("Authorization", "Bearer " + accessToken);
            connection.setRequestProperty("Accept", "application/json");

            String response = HttpClientHelper.getResponseStringFromConnection(connection);
            ...
            liferay.debug(responseObject.toString());

            return responseObject.toString();
        } catch (IOException e) {
            liferay.error("While getting users groups from " + userMembershipUrl + ":: " + e.getMessage());
            return HttpClientHelper.processResponse(500, e.getMessage()).toString();
        }
    }
